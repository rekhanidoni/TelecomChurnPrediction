...
setwd("C:\\MyData\\Upgrad&IIITB\\PredictiveAnalytics\\Module3_Logistic Regression\\Telecom Churn")
# Loading 3 files
...
confusionMatrix(test_pred_churn, test_actual_churn, positive = "Yes")
...
##Calculating gain chart
testdecile1=test[order(test$prob,decreasing = T)[1:211],]
test_pred_churn <- factor(ifelse(testdecile1$prob >=0.3132, "Yes", "No"))
test_actual_churn <- factor(ifelse(testdecile1$Churn==1,"Yes","No"))
confmat=confusionMatrix(test_pred_churn, test_actual_churn, positive = "Yes")
confmat$table[2,2]
trueposCount = vector(length=10,mode="integer")
i=1
churnCalc = function(sidx,eidx) {
  print(as.integer(sidx))
  print(as.integer(eidx))
  testdecile2=test[order(test$prob,decreasing = T)[sidx:eidx],]
  test_pred_churn <- factor(ifelse(testdecile2$prob >=0.3132, "Yes", "No"))
  test_actual_churn <- factor(ifelse(testdecile2$Churn==1,"Yes","No"))
  confmat=confusionMatrix(test_pred_churn, test_actual_churn, positive = "Yes")
  return(confmat$table[2,2])
}

startidx=1
endidx=211
while(endidx <= nrow(test)) {
  trueposCount[i] = churnCalc(startidx,endidx)
  i = i+1
  startidx=endidx+1
  endidx=startidx+210
}
